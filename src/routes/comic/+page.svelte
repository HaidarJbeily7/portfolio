...
<div id="comicContainer">
    {#if comic}
        ...
        <p>
            Published on:
            {new Date(Number(comic.year), Number(comic.month) - 1, Number(comic.day)).toLocaleDateString()}
            ({formatDistanceToNow(new Date(Number(comic.year), Number(comic.month) - 1, Number(comic.day)))} ago)
        </p>
        <button id="get-another-comic" on:click={handleGetAnotherComic}>
            Get Another Comic!
        </button>
    {/if}
</div>
